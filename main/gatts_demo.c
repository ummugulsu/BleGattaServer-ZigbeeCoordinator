...
static char test_device_name[ESP_BLE_ADV_NAME_LEN_MAX] = "GUL2";
...
extern void zb_send_payload_from_ble(const char* json_str);
...
// Global deðiþken (gatts_demo.c en üstüne ekle)
char g_ble_json_buf[256] = { 0 };
...
void parse_json_payload(const uint8_t* data, uint16_t len) {
    ...
    ESP_LOGI(GATTS_TAG, "JSON Received: %s", json_str);

    cJSON* json = cJSON_Parse(json_str);
    if (!json) {
        ESP_LOGE(GATTS_TAG, "Failed to parse JSON");
        free(json_str);
        return;
    }
    ...
    if (cJSON_IsString(led) && (led->valuestring != NULL)) {
        ESP_LOGI(GATTS_TAG, "LED Command: %s", led->valuestring);
    }

    const cJSON* temp = cJSON_GetObjectItemCaseSensitive(json, "temp");
    if (cJSON_IsNumber(temp)) {
        ESP_LOGI(GATTS_TAG, "Temperature Value: %.2f", temp->valuedouble);
    }
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            // BLE'den gelen JSON verisini Zigbee'ye gönder
            char json_buf[256] = { 0 };
            int len = param->write.len;
            if (len > 255) len = 255;
            memcpy(json_buf, param->write.value, len);
            json_buf[len] = '\0';
            zb_send_payload_from_ble(json_buf);
...
void ble_start(void) {
    // Burada app_main içeriðini ble için olan tüm baþlangýç kodlarý ile taþý
    esp_err_t ret;
